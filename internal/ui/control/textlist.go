...
func (tl *TextList) Update() {
	...
	for i, btn := range tl.buttons {
		j := i + offset
		if j < len(tl.items) {
			btn.Label = tl.items[j].GetStats()
			btn.OnClick = func() { tl.OnClick(tl.items[j].GetID()) }
		} else {
			btn.Label = ""
			btn.OnClick = func() { tl.OnClick(0) }
		}
		btn.Update()
	}
...
func (tl *TextList) UpdateItems(items []Statable) {
	tl.items = items
...
func NewTextList(x, y, width, height int, items []Statable) *TextList {
